<?php 

/**
 * Config.Padrao [ HELPER ]
 * Constantes padrão gerados do sistema
 *
 * @copyright (c) 2019, Leo Bessa
 */ 
define('TEM_SITE', '1');
define('TABELA_AUDITORIA', '1');
define('DESC', 'Sistema Interno Retiro');
define('DESC_SIS', 'Web Retiro');
define('TITULO_SITE', 'Sistema de gestão do Retiro');
define('DESC_SITE', 'Fazemos missões e evangelizações diversas para todos os jovens que queira conhecer o verdadeiro amorde Deus.');
define('CONTROLLER_INICIAL_ADMIN', 'Index');
define('ACTION_INICIAL_ADMIN', 'Index');
define('CONTROLLER_INICIAL_SITE', 'IndexWeb');
define('ACTION_INICIAL_SITE', 'Index');
define('TABLE_USER', 'tb_usuario');
define('CAMPO_PASS', 'ds_code');
define('CAMPO_ID', 'co_usuario');
define('CAMPO_PERFIL', 'ds_perfil');
define('SESSION_USER', 'user_gej');
define('INATIVO', '20');
define('BACKUP', '2');
define('TAMANHO', '800');
define('WHATSAPP', '5561981678354');
define('NUM_BG_IMAGENS', '12');
define('CONTROLLER_SEO', '');
define('USER_EMAIL', 'contato@ccaug.com.br');
define('PASS_EMAIL', 'Admin101**');
define('HOST_EMAIL', 'mail.ccaug.com.br');
define('PORTA_EMAIL', '465');
define('ID_ANALITCS', 'UA-71215942-1');
define('MODULO_ASSINANTE', '');
define('LOGAR_EMAIL', false);

