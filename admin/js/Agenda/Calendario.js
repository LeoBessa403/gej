        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                select: function (info) {
                    ...
                    dt_inicio = dia + '-' + mes + '-' + info.start.getFullYear();
                ...
                eventClick: function (info) {
                    if (info.event.id) {
                        $.get(urlValida, {valida: 'pesquisa_agenda', co_agenda: info.event.id},
                            function (retorno) {
                                window.location.href = home + retorno;
                            });
                    } else {
                        var dia = Calendar.VerificaNumero(info.event.start.getDate());
                        var mes = Calendar.VerificaNumero((info.event.start.getMonth() + 1));
                        var hora_inicio = '20-00';
                        var dt_inicio;

                        dt_inicio = dia + '-' + mes + '-' + info.event.start.getFullYear();
